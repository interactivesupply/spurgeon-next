...
// Pattern rules: any legacy redirect whose slug is preserved on the new
// site is covered by one of these. Anything renamed stays as an explicit
// entry above the pattern block. Reduces file size by ~94%.
const PATTERN_RULES = [
...
function isCoveredByPattern(e) {
  for (const { source: src, destination: dst } of PATTERN_RULES) {
    const srcCat = src.split('/')[2];
    const dstPrefix = dst.split('/')[1];
    const m1 = e.source.match(new RegExp(`^/resource-library/${srcCat}/([^/]+)$`));
    const m2 = e.destination.match(new RegExp(`^/${dstPrefix}/(.+)$`));
    if (m1 && m2 && m1[1] === m2[1]) return true;
  }
  return false;
}
const explicitRedirects = redirects.filter((e) => !isCoveredByPattern(e));
const collapsed = redirects.length - explicitRedirects.length;
...
//
// Order matters! Explicit slug-rename redirects come first so that they
...
// (${collapsed} identity redirects were collapsed into the ${PATTERN_RULES.length} pattern rules
//  below — saves ~${Math.round((1 - (explicitRedirects.length + PATTERN_RULES.length) / redirects.length) * 100)}% of the entries.)
//
...
`;
const footer = `];\n`;
const explicitBody = explicitRedirects
  .map(r => `  { source: ${JSON.stringify(r.source)}, destination: ${JSON.stringify(r.destination)}, permanent: true },`)
  .join('\n');
const patternBody = PATTERN_RULES
  .map(r => `  { source: ${JSON.stringify(r.source)}, destination: ${JSON.stringify(r.destination)}, permanent: true },`)
  .join('\n');
const body =
  `  // ---- Explicit slug renames (${explicitRedirects.length} entries) ----\n` +
  explicitBody +
