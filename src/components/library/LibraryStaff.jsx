...
const FALLBACK_DIRECTORS = [
  {
    name: "Dr. Geoff Chang",
    role: "Curator of the Spurgeon Library",
    affiliation: "MBTS",
  ...
  },
  {
    ...
    role: "General Editor and Fellow of the Spurgeon Library",
    affiliation: "MBTS",
    url: "https://www.mbts.edu",
...
const FALLBACK_RESEARCH_ASSISTANTS = ["Research Assistant", "Research Assistant", "Research Assistant"];

const FALLBACK_PRESIDENT = {
  eyebrow: "From Our President",
  heading: "A Word from Dr. Jason Allen",
  photo: "https://media.base44.com/images/public/699e34d59ad598edd05d1adb/7c8de5f4e_jkallen_headshot_square_1200-e1743173887118-1060x710.jpg",
  quote: "As owners of the Spurgeon Library, Midwestern enjoys a singular stewardship to our students, our denomination, and the church at large. The Charles Spurgeon Lectures on Biblical Preaching equip preachers and gospel ministers and fulfill our mission to exist for the church. The goal of the lectures is to equip attendees and serve the church by speaking on and modeling expositional preaching.",
  attribution: "Dr. Jason Allen, President, Midwestern Baptist Theological Seminary",
};

/**
 * Receives staff[] (split into director + research_assistant via the staff_type
 * ACF field) and a `president` block (eyebrow, heading, photo, quote, attribution).
 * Falls back to hardcoded data when WP isn't available.
 */
export default function LibraryStaff({ staff, president }) {
  const directors = (staff?.length
    ? staff.filter((s) => s.type === "director")
    : FALLBACK_DIRECTORS);
  const researchAssistants = (staff?.length
    ? staff.filter((s) => s.type === "research_assistant").map((s) => s.name)
    : FALLBACK_RESEARCH_ASSISTANTS);
  const pres = president && president.quote ? president : FALLBACK_PRESIDENT;
    ...
    <div className="bg-card border-t border-b border-border">
      <div className="max-w-5xl mx-auto px-6 py-16 md:py-20">
              ...
              <motion.div
                ...
                {person.photo && (
                  <img
                    src={person.photo}
                    alt={person.name}
                    className="rounded-xl object-cover object-top shadow-lg shrink-0 relative z-10 -mr-8"
                    style={{ width: '6.5rem', height: '6.5rem' }} />
                )}
                <div className="bg-background border border-border rounded-2xl pl-12 pr-6 py-5 flex-1">
                  <h3 className="font-serif text-lg font-semibold text-foreground">{decodeEntities(person.name)}</h3>
                  {person.role && (
                    <p className="font-sans text-sm text-muted-foreground mt-0.5">{decodeEntities(person.role)}</p>
                  )}
                  {person.url && person.affiliation && (
                    <a
                      href={person.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 mt-2 font-sans text-xs text-accent hover:text-accent/80 transition-colors">
                      {decodeEntities(person.affiliation)}
        ...
        <motion.div
          ...
          <p className="font-sans text-xs tracking-[0.2em] uppercase text-muted-foreground mb-4 text-center">{decodeEntities(pres.eyebrow)}</p>
          <h3 className="font-serif text-xl font-semibold text-foreground mb-6 text-center">{decodeEntities(pres.heading)}</h3>
          <div className="max-w-4xl mx-auto relative flex flex-col md:flex-row items-center">
            {pres.photo && (
              <img
                src={pres.photo}
                alt={pres.attribution}
